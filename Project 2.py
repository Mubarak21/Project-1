...
Groceries= [ "Apples" , "Mango","Watermelon"]# The list containing the groceries
Price= [1.25, 2.9,5]# It contains the prices of the above groceries
...
def fruits(Groceries, Price):# A function that takes the groceries and TotalPrice and calculates the total price
    for i in range (len(Groceries)):# This loops is because there only 4 fruits in the list so it runs for that many times
        ...
        print("try again")
        option= input("Do you wanna try again?, Y or N")
choice= "Y"
while choice != "N":# A loop sothat the user can keep adding this to his list
            fruit= input ('what fruit do you want?, either  Mango, Apples or Watermelon')# Asks the user what fruit they prefer
...
if receipt == "Yes":
                    print("The total amount of all the fruits that you bought is",sum(TotalPrice), " Dirhams")# The final message displaying the Total money need for the groceries
